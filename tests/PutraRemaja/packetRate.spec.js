...
async function packetRate(webApp) {
    ...
    const packetRatePath = webApp.locator(`xpath=//a[normalize-space()='Paket']`).first();
    await packetRatePath.click();
    await webApp.locator(`xpath=//div[normalize-space()='Masukkan data paket']`).click();

    // Expect the page to have text cek tarif kirim paket
    await expect(webApp.locator(`xpath=//h5[@id='modalTarifPaketLabel']`)).toBeVisible();
}
...
async function pickOrigin(webApp, origin) {
    ...
    const originDropdown = webApp.locator(`xpath=//span[normalize-space()='Lokasi Asal']`);
...
async function pickType(webApp, packetType) {
    ...
    const typeDropdown = webApp.locator(`xpath=//span[normalize-space()='Jenis Paket']`);
...
async function setHeight(webApp, height) {
    test.info().annotations.push({
        ...
        await webApp.locator(`xpath=//button[normalize-space()='Cek Tarif']`).click();
}
