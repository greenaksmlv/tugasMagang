...
async function packetRate(webApp) {
    ...
    const packetRatePath = webApp.locator(`xpath=//input[@placeholder='Masukan Data Paket']`);
    await packetRatePath.click();

    // Expect the page to have text cek tarif kirim paket
    await expect(webApp.locator(`xpath=//div[@class='d-flex justify-content-between mb-2 px-3']//h5[@id='exampleModalLongTitle']`)).toBeVisible();
}
...
async function pickOrigin(webApp, origin) {
    ...
    const dropdownWrapper = webApp.locator(`xpath=//span[normalize-space()='Lokasi Asal']`);
    await expect(dropdownWrapper).toBeVisible({ timeout: 10000 });
    await dropdownWrapper.click();

    const originOption = webApp.locator(`xpath=//div[@class='ss-option'][normalize-space()='${origin}']`)
...
//     const typeDropdown = webApp.locator(`xpath=//div[@class='ss-48264 ss-main form-control-reservasi card-input-cek']//span[@class='placeholder']`);
...
async function setWeight(webApp, weight) {
    test.info().annotations.push({
        ...
        const weightInput = webApp.locator(`xpath=//div[@class='form-group d-flex']//input[@id='berat']`);
...
async function setLength(webApp, length) {
    test.info().annotations.push({
        ...
        const lengthInput = webApp.locator(`xpath=//div[@class='form-group d-flex']//input[@id='panjang']`);
...
async function setWidth(webApp, width) {
    test.info().annotations.push({
        ...
        const widthInput = webApp.locator(`xpath=//div[@class='form-group d-flex']//input[@id='lebar']`);
...
async function setHeight(webApp, height) {
    test.info().annotations.push({
        ...
        const heightInput = webApp.locator(`xpath=//div[@class='form-group d-flex']//input[@id='tinggi']`);
        await expect(heightInput).toBeVisible();
        await heightInput.fill(height.toString());

        await webApp.locator(`xpath=//button[@class='btn first-bg-color w-100 br-16 text-white']`).click();
}
