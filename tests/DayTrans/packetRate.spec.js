...
/**
 * Fungsi:
 * - Navigasi ke halaman "Tarif Paket" dari menu dropdown
 * 
 * Alur:
 * - Klik dropdown menu "Paket"
 * - Klik item menu "Tarif Paket"
...
async function packetRate(webApp) {
    ...
    const packetRatePath = webApp.locator(`xpath=//a[normalize-space()='Tarif Paket']`);
    await packetRatePath.click();

    // Expect the page to have text cek tarif kirim paket
    await expect(webApp.locator(`xpath=//h3[contains(text(),'Cek')]`)).toBeVisible();
}
...
async function pickOrigin(webApp, origin) {
    ...
    const originDropdown = webApp.locator(`xpath=//span[normalize-space()='Masukkan kota asal pengiriman']`);
    await expect(originDropdown).toBeVisible();
    await webApp.waitForTimeout(500);
    await originDropdown.click();

    const originOption = webApp.locator(`xpath=//div[normalize-space()='${origin}']`);
...
async function pickDestination(webApp, destination) {
    ...
    const destinationDropdown = webApp.locator(`xpath=//span[normalize-space()='Masukkan kota tujuan']`);
    await expect(destinationDropdown).toBeVisible();
    await webApp.waitForTimeout(500);    
    await destinationDropdown.click();

    const destinationOption = webApp.locator(`xpath=//div[@class='ss-content ss-open']//div[@class='ss-option'][normalize-space()='${destination}']`);
...
async function pickType(webApp, packetType) {
    ...
    const typeDropdown = webApp.locator(`xpath=//span[normalize-space()='Pilih jenis paket']`);
...
/**
 * Fungsi:
 * - Memilih volume paket dari dropdown
 * 
 * Alur:
 * - Klik dropdown volume paket
 * - Pilih volume dari parameter
 * 
 * @param {object} webApp - Objek browser Playwright
 * @param {string} packetVolume - Volume paket
 */

// Helper function to pick packet volume
async function pickVolume(webApp, packetVolume) {
test.info().annotations.push({
        type: 'allure.step',
        value: 'Pick packet destination',
    });
    
    const volumeDropdown = webApp.locator(`xpath=//span[normalize-space()='Pilih volume paket']`);
    await expect(volumeDropdown).toBeVisible();
    await webApp.waitForTimeout(500);
    await volumeDropdown.click();

    const volumeOption = webApp.locator(`xpath=//div[normalize-space()='${packetVolume}']`);
...
async function setHeight(webApp, height) {
    test.info().annotations.push({
        ...
        await webApp.locator(`xpath=//button[normalize-space()='Cek Harga']`).click();
}
...
test('packet rate', async ({ webApp }) => {
    ...
    // Pick packet volume
    await pickVolume(webApp, config.packet.packetVolume);
