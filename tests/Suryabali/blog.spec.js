...
async function blog(webApp) {
    ...
    const blogPath = webApp.locator(`xpath=//a[contains(@class,'nav-link')][normalize-space()='Blog']`);
    await blogPath.isVisible();
    await blogPath.click();

    // Expect the page to have text
    await expect(webApp.locator(`xpath=//h1[normalize-space()='Promo dan Berita Surya Bali']`)).toBeVisible(); 
}
